...
FROM steep_repl.capture_fingerprint('public', 'fp_test_table');

-- Verify it's stored
SELECT table_schema, table_name, fingerprint IS NOT NULL AS has_fingerprint, column_count
FROM steep_repl.schema_fingerprints
WHERE table_schema = 'public' AND table_name = 'fp_test_table';
...
-- Test capture_all_fingerprints returns a count
SELECT steep_repl.capture_all_fingerprints() > 0 AS captured_tables;

-- Cleanup
DELETE FROM steep_repl.schema_fingerprints WHERE table_schema = 'public' AND table_name = 'fp_test_table';
