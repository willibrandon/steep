    ...
    fn test_schema_fingerprints_column_definitions_jsonb() {
        Spi::run(
            r#"INSERT INTO steep_repl.schema_fingerprints
               ...
               VALUES ('public', 'test_jsonb', 'def456abc123', 2,
        ...
        let result = Spi::get_one::<i32>(
            "SELECT jsonb_array_length(column_definitions) FROM steep_repl.schema_fingerprints
             WHERE table_schema = 'public' AND table_name = 'test_jsonb'"
        );
        assert_eq!(result, Ok(Some(2)));

        // Cleanup
        Spi::run("DELETE FROM steep_repl.schema_fingerprints WHERE table_schema = 'public' AND table_name = 'test_jsonb'")
            .expect("cleanup should succeed");
    }
}
