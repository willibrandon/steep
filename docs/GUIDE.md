...
**Spec-Kit Command**:
```bash
/speckit.specify Implement Dashboard and Activity Monitoring view for Steep. CRITICAL REQUIREMENTS BEFORE IMPLEMENTATION: 1) Study reference tools (pg_top, htop, k9s) - clone repos, run them, screenshot dashboards. Include screenshots in spec.md. 2) Create ASCII mockup of exact dashboard layout in spec.md showing what it should look like character-by-character. 3) Evaluate graphing libraries - build 3 throwaway demos testing different approaches (unicode blocks, bubbles components, simple text-based bars). Include screenshots of each demo in spec.md. Choose approach that looks best. 4) Define visual acceptance criteria: "Dashboard must look like [reference tool X] with clean borders, aligned text, stable graphs". 5) Build static mockup with hardcoded data first, get visual approval, THEN add real-time data. IMPLEMENTATION: Dashboard shows TPS, cache hit ratio, connection count, database size. Activity view shows connections table (PID, user, database, state, duration, query). Auto-refresh 1s. Actions: kill connections, view full query, filter. Query pg_stat_activity and pg_stat_database under 500ms. Prioritize P1 (viewing) over P2 (actions).
```
